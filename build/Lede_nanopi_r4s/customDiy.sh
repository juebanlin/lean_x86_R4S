...
#sed -i 's/sources.openwrt.org/sources.cdn.openwrt.org/g' scripts/download.pl
...
# 版本号里显示一个自己的名字（juebanlin build $(TZ=UTC-8 date "+%Y.%m.%d") @ 这些都是后增加的）
sed -i "s/OpenWrt /juebanlin build@OpenWrt /g" package/lean/default-settings/files/zzz-default-settings
